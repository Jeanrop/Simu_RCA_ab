...
Path_save = r'/home/jeanrop/Documents/data/h5_full'
...
#%% Creation of one H5 file

Path_h5 = os.path.join(Path_save, str(N_simu_tot) + '_simus.h5')
...
with h5py.File(Path_h5, "a", libver='latest') as hf:
    for job_folder in job_folders:
        idx_job += 1
        print("Folder simu number ", idx_job, " out of ", str(N_job))
        Path_folder = os.path.join(Path_load, job_folder)
        simu_folders = [folder for folder in os.listdir(Path_folder) if os.path.isdir(os.path.join(Path_folder, folder))]
        Path_h5 = os.path.join(Path_save, 'simu_100.h5') 
        for simu_folder in simu_folders:
            idx_simu += 1
            print("Simu number ", idx_simu, " out of ", str(N_simu_tot))
            Path_simu = os.path.join(Path_folder, simu_folder)
            for name in var_names:
