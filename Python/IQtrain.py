...
def main(raw_args=None):
    ...
    parser.add_argument('config_path', type=str, help='path to the config file for the training')
    args = parser.parse_args(raw_args)

    with open(args.config_path) as json_file:
        config = json.load(json_file)
    n_epochs = config['epoch']
    # check and default the config file, keep it compliant with the change in the config file
