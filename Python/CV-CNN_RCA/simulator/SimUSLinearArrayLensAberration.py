...
def SimUSLinearArrayLensAberration(r_scat, bsc, Delays, Apod, DelaysAberration, AmplitudeAberration, psim):
    """
    Parameters (new)
    ----------------
    DelaysAberration : ndarray
        contains the time delays of the aberration for each element (in seconds)
    AmplitudeAberration : ndarray
        contains the amplitude of the aberration for each element (in residual %)
    """
    ...
    for index in range(ntx):
        ...
        #convert the time and amplitude aberration to array fire
        deAb = af.from_ndarray(DelaysAberration[index])
        ampAb = af.from_ndarray(AmplitudeAberration[index])
        deAb=af.tile(af.reorder(deAb,1,0),1,neleHeight)
        ampAb=af.tile(af.reorder(ampAb,1,0),1,neleHeight)

        s_tx = af.tile(ap, Nf) *af.exp(1j * af.matmul(wi, de))

        #calculate the aberration as phase and amplitude
        A_tx=af.tile(ampAb,Nf)* af.exp(1j * af.matmul(wi,deAb))
        ...
        for iframe in range(nframe):
            ...
            #-- Transmit
            #add aberration in transmit
            TX = af.sum(G_tx * D_tx * af.tile(s_tx *A_tx/neleTotal, 1, 1, nscatPerEvent), dim=1)


            #-- Receive [ki ele scat]
            #add aberration in receive
            RX = af.tile(af.reorder(bsc_tmp,2,0,1),Nf,neleTotal) * G_tx * D_tx * af.tile(A_tx,1,1,nscatPerEvent)
