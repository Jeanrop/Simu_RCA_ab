SAVE_ID = getenv("SAVE_ID");
tic
pause(str2double(SAVE_ID)/100);
a = toc*10000;
rng(a)
warning("off")
...
load(['/home/jeanrop/scratch/Pos_Steph_100/data_stack_', SAVE_ID])
...
clear PosX PosY
for i_simu = 1:100
    NbbPerROI = round(unifrnd(350, 500));
    i_bubble = round(unifrnd(1, NbbPerROI));
    ...
    for i=1:NframePerBuffer
        ...
        Pos_bubble (:,i)  = Pos(i_bubble,:,1+(i-1)*P.nbAngles);
    ...
    for iFrame = 1:16
        ...
        dataBfY_al        = squeeze(dataBfY);

        dataBf_al(:,:,:,iFrame) = cat(3,dataBfX_al(:,:,:), dataBfY_al(:,:,:));
        ...
        [dataBfXab_c,maskXab] = bfVirtualSourceGeneric_RCA_Nosum_ab(...
            ...
            GeometryX2(iFrame), TransmitX, Param, Options);
        dataBfX_al_ab_c        = squeeze(dataBfXab_c);
        ...
        [dataBfYab_c,maskYab] = bfVirtualSourceGeneric_RCA_Nosum_ab(...
            ...
            GeometryY2(iFrame), TransmitY, Param, Options);
        dataBfY_al_ab_c        = squeeze(dataBfYab_c);

        dataBf_al_ab_c(:,:,:,iFrame) = cat(3,dataBfX_al_ab_c, dataBfY_al_ab_c);
        ...
        [dataBfXab,maskXab] = bfVirtualSourceGeneric_RCA_Nosum(...
            reshape(RF_X_ab(:,:,:,iFrame),size(RF_X_ab,1),128,1,[]),...
            GeometryX2(iFrame), TransmitX, Param, Options);
        dataBfX_al_ab        = squeeze(dataBfXab);

        [dataBfYab,maskYab] = bfVirtualSourceGeneric_RCA_Nosum(...
            reshape(RF_Y_ab(:,:,:,iFrame),size(RF_Y_ab,1),128,1,[]),...
            GeometryY2(iFrame), TransmitY, Param, Options);
        dataBfY_al_ab        = squeeze(dataBfYab);

        dataBf_al_ab(:,:,:,iFrame) = cat(3,dataBfX_al_ab, dataBfY_al_ab);
    end

    %% Save mat files
    Path_save = ['/home/jeanrop/scratch/10_job_100_simu_2/', SAVE_ID '/' num2str(i_simu)];
    mkdir(Path_save)

    % data BF
    save([Path_save, '/dataBf_al.mat'], 'dataBf_al', "-nocompression")
    save([Path_save, '/dataBf_al_ab.mat'], 'dataBf_al_ab', "-nocompression" )
    save([Path_save, '/dataBf_al_ab_c.mat'], 'dataBf_al_ab_c', "-nocompression")

    save([Path_save, '/DelaysAb.mat'], 'DelaysAb', "-nocompression")
    save([Path_save, '/Pos.mat'], 'Pos', "-nocompression" )
    save([Path_save, '/Pos_bubble.mat'], 'Pos_bubble', "-nocompression")
end