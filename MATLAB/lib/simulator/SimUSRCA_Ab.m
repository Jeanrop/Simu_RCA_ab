function [RF,psim]   = SimUSRCA_V2(Pos,BSC,Delays,DelaysAb,AmpAb,Apod,psim)
...
BSC      = single(BSC); 
Apod     = single(Apod); 
Delays   = single(Delays) + psim.t0;
DelaysAb = single(DelaysAb);
AmpAb    = single(AmpAb);
%% BEGIN
% Element position
...
switch psim.gpu
    case 0 % 0 = cpu
        ...
        for itx = 1:Ntx
            ...
            %-- Receive
            RX_ab    = permute(AmpAb(:).*exp(1j.*DelaysAb(:).*wi),[3 1 2]); %
            RX      = BSC(:,itx).*D_tx.*RX_ab;  % Nscatter * Nele * Nki 
