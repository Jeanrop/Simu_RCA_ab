function [dataBf, mask] = bfVirtualSourceGenericNosum(varargin)
% BFVIRTUALSOURCEGENERIC 3D/2D/1D I/Q beamformer for virtual sources behind the array.
%     [dataBf, mask] = bfVirtualSourceGeneric(data, Geometry, Transmit, Param)
%      dataBf        = bfVirtualSourceGeneric(data, Geometry, Transmit, Param)
%     [dataBf, mask] = bfVirtualSourceGeneric(data, Geometry, Transmit, Param, Options)
%      dataBf        = bfVirtualSourceGeneric(data, Geometry, Transmit, Param, Options)
...
%
%     INPUTS:
%         data    : raw data [samples x x-channels x y-channels x events]
...
%         for you. 
%         Run "bfVirtualSourceGeneric()" before first use.
%
...
for s = 1:length(minDistVirtualSource)
    minDistVirtualSource(s) = min(sqrt( ...
                          (posEleX-virtualSourceX(s)).^2+ ...
                          (posEleY-virtualSourceY(s)).^2+ ...
...
%% DO BF WITH CUDA
%--- CUDA kernel
cudaKernel = parallel.gpu.CUDAKernel('bfVirtualSourceGenericNosum.ptx',...
    'bfVirtualSourceGenericNosum.cu',...
    'bfVirtualSourceGenericNosum'); % Create CUDA object

cudaKernel.ThreadBlockSize = [768 1 1]; % Thread (1024). DO NOT CHANGE.
...
% else
dataBf = zeros(nPix,nTx, nFrame, nChannel, 'single'); % Beamformed data without compounding
% end

maskGPU   = gpuArray(zeros(nPix, nTx, 'single')); % Mask  (gpu array)
dataBfGPU = gpuArray(zeros(2*nPix,nTx, nFrameMax,nChannel, 'single')); % Beamformed data (gpu array)
...
dataBf = reshape(dataBf, ...
    ...
    [], nFrame,nChannel);  % Beamformed data
...
function compilePTX
    ...
    errStr.message = ['Microsoft Visual Studio not found! ', ...
        ...
        'system(''nvcc -ptx bfVirtualSourceGeneric.cu -use_fast_math', ...
    ...
    system(['nvcc -ptx bfVirtualSourceGenericNosum.cu -use_fast_math -ccbin ', ...
            '"' cudaSelected '"']);
