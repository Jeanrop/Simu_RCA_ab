function [dataBf, mask] = bfVirtualSourceGenericTxRx(varargin)
...
AbLawTx             = single(Param.AbLawTx);
AbLawRx             = single(Param.AbLawRx);
...
%% DO BF WITH CUDA
%--- CUDA kernel
cudaKernel = parallel.gpu.CUDAKernel('bfVirtualSourceGenericTxRx.ptx',...
    'bfVirtualSourceGenericTxRx.cu',...
    'bfVirtualSourceGenericTxRx'); % Create CUDA object
...
function compilePTX
    ...
    system(['nvcc -ptx bfVirtualSourceGenericTxRx.cu -use_fast_math -ccbin ', ...
            '"' cudaSelected '"']);
