...
// MAIN FUNCTION
__global__ void bfVirtualSourceGenericTxRx(
              float *dataBf,
              float *mask,
        const float *data,
        const float *gridBfX,
...

{
    ...
    if (idxFrame<nFrame) // If frame index is valid
    {
        ...
        if (maskTransmit)
		{
			...
			// Delay in transmit
            delayTx = (norm3df(pixX - virtualSourceXTx,
			...
			for (int idxChannel = 0; idxChannel < nChannel; idxChannel++)
			{
				...
				int idxPixAb = idxPix + idxChannel*nPix; // Aberration pixel index
				...
				float AbLawRxPix = AbLawRx[idxPixAb];
                float AbLawTxPix = AbLawTx[idxPixAb];
				...
				if (aperture<apertureF) // If the aperture is valid
				{
					...
					// Delays
					delayRx  =  AbLawRxPix + hypotf(aperture, pixZ - eleZ) *
									  invSpeedOfSound; // In receive
					idxDelay = (delayTx + delayRx + startTime + AbLawTxPix) *
