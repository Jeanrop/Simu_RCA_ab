/* 
I/Q beamformer for flat angles, RCA probe.
...
Parameters:
    ...
    [13+] flagReceiveX:  flag for receive channels (flag=1 receive on X channels, flag=0 receive on Y channels)
...
{
	// CUDA intrinsic functions are faster than standard ones here, double precision is needed for intermediate terms (to avoid precision errors)
	double abx = bx-ax;
	double aby = by-ay;
	double acx = cx-ax;
	double acy = cy-ay;
...
// MAIN FUNCTION      
__global__ void bfVirtualSourceGeneric_RCA_ab_2(
              float *dataBf,
              float *mask,
        const float *data,
        const float *gridBfX,	
...

{
    ...
    if (idxFrame<nFrame) // If frame index is valid
    {
        ...
        if (maskTransmit)
		{
			...
			int idxPixBf = 2*(idxPix + idxTx*nPix + idxFrame*nPix*nTx); // Output pixel index = frame-Tx-pixel index 
			...
			for (int idxChannel = 0; idxChannel < nChannel; idxChannel++)
			{
				...
				eleX = posEleX[idxChannel]; // X position of the element of interest in receive
				eleY = posEleY[idxChannel]; // Y position of the element of interest in receive
				eleZ = posEleZ[idxChannel]; // Z position of the element of interest in receive
  
						
				//aperture = hypotf(pixX - eleX, pixY - eleY); // Aperture
				...
				if (aperture<apertureF) // If the aperture is valid
				{
                    ...
                    // Loi aberration - Depend que de l'element de reception - hypothese simple 
                    float AbLawRx_idx = AbLawRx[idxTx*nChannel + idxChannel];

					// Delays in receive
					delayRx  =  AbLawRx_idx + hypotf(aperture, pixZ - eleZ)  * 
									  invSpeedOfSound; 
